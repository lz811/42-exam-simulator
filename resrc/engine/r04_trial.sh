...
topic_pool() {
    case "$stage" in
        level1)
            echo "list_intersection_finder merge_sorted_lists"
            ;;
        level2)
            echo "array_rotation_detector constellation_mapper"
            ;;
        level3)
            echo "palindrome_partitioner sliding_window_maximum"
            ;;
        level4)
            echo "package_dependency_resolver"
            ;;
        *)
            echo ""
            ;;
    ...
    case "$input" in
        test)
            ...
            if echo "$result" | grep -q "PASSED"; then
                ...
                echo -e "${FG_YELLOW}${TXT_BOLD}✦ $(epic_praise)${TXT_RESET}"
                rm -f "$pick_cache"
                sleep 2
                exit 0
            else
